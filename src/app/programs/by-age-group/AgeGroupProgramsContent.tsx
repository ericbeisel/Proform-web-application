...
import { getProgramsByAgeGroup, Program } from "@/api/programs/route";
...
export default function AgeGroupProgramsContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const ageGroupId = searchParams.get("ageGroupId") || "";
  const ageGroupName = searchParams.get("ageGroupName") || "Age Group";
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      try {
        setLoading(true);
        const response = await getProgramsByAgeGroup(ageGroupId, currentPage);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 capitalize">
                {decodeURIComponent(ageGroupName)} Programs
              </h1>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        {programs.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No programs found for {decodeURIComponent(ageGroupName)}.</p>
            <button
              onClick={() => router.push("/programs/all-programs")}
