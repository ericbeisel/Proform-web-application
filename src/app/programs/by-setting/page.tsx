...
import { ArrowLeft, Loader2, ChevronLeft, ChevronRight, Search, Users } from "lucide-react";
import { getProgramsBySetting, Program } from "@/api/programs/route";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-3 flex flex-col">
        ...
        {/* Sport if available */}
        {(program as any).sport && (program as any).sport !== "" && (
          <p className="text-xs text-purple-500 mt-1">⚽ {(program as any).sport}</p>
        )}
        ...
        {/* Featured Trainers */}
        {(program as any).featured_trainers && (program as any).featured_trainers !== "" && (
        <p className="text-xs text-purple-500 mt-2 line-clamp-1 flex items-center gap-1">
    <Users size={14} className="text-purple-500" />
    {(program as any).featured_trainers}
  </p>
        )}
      </div>
    </div>
...
export default function SettingProgramsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const settingId = searchParams.get("settingId") || "";
  const settingName = searchParams.get("settingName") || "Setting";
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      try {
        setLoading(true);
        const response = await getProgramsBySetting(settingId, currentPage);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 capitalize">
                {decodeURIComponent(settingName)} Programs
              </h1>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        {programs.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No programs found for {decodeURIComponent(settingName)}.</p>
            <button
onClick={() => router.push("/programs/all-programs")}
