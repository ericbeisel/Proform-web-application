...
import { getProgramsByFocus, Program } from "@/api/programs/route";
...
export default function FocusProgramsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const focusId = searchParams.get("focusId") || "";
  const focusName = searchParams.get("focusName") || "Focus";
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      try {
        setLoading(true);
        const response = await getProgramsByFocus(focusId, currentPage);
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        {programs.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No programs found for {focusName}.</p>
            <button
onClick={() => router.push("/programs/all-programs")}
