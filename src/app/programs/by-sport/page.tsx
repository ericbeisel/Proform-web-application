...
import { getProgramsBySport, Program } from "@/api/programs/route";
  ...
  return (
    ...
    >
      <div className="relative h-44 overflow-hidden">
        ...
        {/* Paid plan image instead of text */}
...
export default function SportProgramsPage() {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        setLoading(true);
        const response = await getProgramsBySport(sport, currentPage);
  ...
  const formatSportName = (sportName: string) => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 capitalize">
                {formatSportName(sport)} Programs
              </h1>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        {programs.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No programs found for {formatSportName(sport)}.</p>
            <button
              onClick={() => router.push("/programs/all-programs")}
          ...
          <>
            {/* Programs Grid - Cards will have consistent height */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
