  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="px-4 md:px-8 py-6 max-w-screen-xl mx-auto space-y-8">
          {/* banner */}
          <SkeletonBox className="h-40 md:h-56 w-full rounded-2xl" />
          {/* 2-col row */}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid grid-cols-3 gap-3">
              {Array.from({ length: 3 }).map((_, i) => <SkeletonBox key={i} className="aspect-square" />)}
            </div>
            <div className="grid grid-cols-3 gap-3">
              {Array.from({ length: 3 }).map((_, i) => <SkeletonBox key={i} className="aspect-square" />)}
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent">
        <p className="text-white text-xs md:text-sm font-bold truncate">{program.title}</p>
        <p className="text-white/80 text-[10px] md:text-xs">{program.duration}</p>
      </div>
    </div>
  );
}
...
export default function SearchWorkoutsPage() {
  ...
  const [loading, setLoading] = useState(!workoutPageCache);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 md:px-8 py-3 md:py-4">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between relative">
          <h1 className="text-lg md:text-xl font-bold text-gray-900 z-10">Search Workouts</h1>
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            ...
            >
              {/* logo loads eagerly — it's above the fold */}
              <img
      ...
      <main className="flex-1 px-4 md:px-8 py-6 md:py-8 max-w-screen-xl mx-auto w-full space-y-8 md:space-y-10">
        ...
        {data.featuredPrograms && (
          <div
            ...
            onClick={() => router.push(`/programs/${data.featuredPrograms?.id}`)}
          >
            <img
            ...
            <div className="relative z-10 p-4 md:p-8 h-full flex flex-col justify-end">
              ...
              <p className="text-white text-lg md:text-2xl font-bold mt-1">{data.featuredPrograms.title}</p>
              <p className="text-gray-300 text-xs md:text-sm">{data.featuredPrograms.duration}</p>
              <p className="text-gray-400 text-xs mt-1 line-clamp-1">{data.featuredPrograms.description}</p>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-400 text-[10px] md:text-xs mb-3">Exclusive partner gyms & studios</p>
    ...
    {data.sportCategories.slice(0, 8).map((sport) => (
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <SectionHeader title="Most Popular" showSeeAll />
              ...
              {data.popularPrograms.slice(0, 3).map((program) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 text-white">
                    <p className="font-bold text-sm md:text-base line-clamp-1">{program.title}</p>
                    <p className="text-xs text-white/90">Week {program.duration || "1"}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-1 md:gap-2 mb-1">
                    ...
                    <span className="text-white text-base md:text-lg font-bold">{data.featuredFranchise.abbreviation}</span>
                  </div>
                  <p className="text-white text-sm font-semibold">{data.featuredFranchise.title}</p>
                  <p className="text-gray-300 text-xs mt-1">{data.featuredFranchise.upgrade_lightbox}</p>
                </div>
              </div>
            </div>
          )}
      ...
      >
        ...
        <span className="text-xs md:text-sm font-semibold text-gray-700 text-center">{setting.title}</span>
          ...
          <div>
            ...
            <button className="relative w-full rounded-2xl flex flex-col items-center justify-center gap-2 md:gap-3 py-10 md:py-1 text-white hover:opacity-95 transition overflow-hidden group h-[270px] min-h-[200px]">
              <div
                className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
                style={{
                  backgroundImage:
                    "url('https://proformapp-storage.s3.eu-north-1.amazonaws.com/Sections+Images/movement.png')",
                }}
              />
              <div
                className="absolute inset-0"
                style={{ background: "linear-gradient(135deg, rgba(108,58,234,0.65) 0%, rgba(139,92,246,0.65) 100%)" }}
              />
              <div className="relative z-10 flex flex-col items-center justify-center gap-2 md:gap-3">
                <Search size={28} className="opacity-95" />
                <span className="font-bold text-lg md:text-xl">Exercise Search</span>
                <span className="text-purple-100 text-xs md:text-sm">Find a workout with a specific movement</span>
              </div>
            </button>
          </div>
        </div>
