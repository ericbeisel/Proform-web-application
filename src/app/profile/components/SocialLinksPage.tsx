...
export default function SocialLinksPage({ onClose }: SocialLinksPageProps) {
  ...
  return (
    ...
    >
      {/* Header – almost identical to original on desktop */}
      <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-gray-100 bg-white rounded-t-2xl">
        <div className="flex items-center gap-3">
          {onClose && (
            <button
              onClick={onClose}
              className="p-1 text-gray-500 hover:text-gray-700 transition-colors sm:hidden md:block"
              aria-label="Go back"
            >
              <ChevronLeft size={22} />
            </button>
        ...
        <button
          onClick={onClose}
          className="bg-purple-700 hover:bg-purple-800 text-white text-[13px] font-bold px-5 py-2 rounded-xl transition-colors mr-9"
        >
          Done
        </button>
      </div>

      {/* Grid – desktop stays 2 columns like original, mobile → 1 column */}
          ...
          {SOCIAL_PLATFORMS.map((platform) => (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <div
                  className={`w-10 h-10 rounded-full ${platform.iconBg} flex items-center justify-center flex-shrink-0`}
                >
                  {platform.icon}
                </div>
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] text-gray-700 placeholder-gray-400 focus:outline-none focus:border-purple-300 focus:ring-2 focus:ring-purple-50 bg-gray-50 transition"
              />

              <button className="mt-2.5 text-[11px] text-gray-400 hover:text-gray-600 transition-colors block">
                Hide from profile
              </button>
            </div>
          ))}
