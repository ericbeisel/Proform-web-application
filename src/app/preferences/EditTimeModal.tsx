...
const EditTimeModal = ({
  ...
  const getSectionDefaultTime = () => {
    const sectionKey = title.toLowerCase().includes("workout")
      ...
      ? "conditioning"
      : null;
    return sectionKey ? DEFAULT_TIMES_BY_SECTION[sectionKey] : "09:00";
  };

  const openPopup = (day: string) => {
    const newTimes = { ...selectedTimes };
    if (!newTimes[day] || newTimes[day].length === 0) {
      const raw = getSectionDefaultTime();
      const { hour, minute, period } = parseTimeToComponents(raw);
      newTimes[day] = [{ startTime: componentsToTimeString(hour, minute, period) }];
  ...
  const addTimeSlot = (day: string) => {
    if ((selectedTimes[day]?.length || 0) < 3) {
      ...
      newTimes[day] = [
        ...(newTimes[day] || []),
        { startTime: componentsToTimeString(hour, minute, period) },
      ];
      setSelectedTimes(newTimes);
    }
  };
  ...
  const removeTimeSlot = (day: string, index: number) => {
    ...
    // Clean up empty days so they don't stay "selected"
    if (updated.length === 0) {
      delete newTimes[day];
    } else {
      newTimes[day] = updated;
    }
    // Clear error when a slot is removed
    ...
    <div className="flex flex-col min-h-screen bg-white w-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-8 md:px-8 lg:px-10 mx-auto w-full max-w-[1400px]">
        ...
        <p className="text-[14px] text-[#666] font-medium leading-relaxed mb-6">
          Select days and add up to 3 workout times per day.
        </p>
