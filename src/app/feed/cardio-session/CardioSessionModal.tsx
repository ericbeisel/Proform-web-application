...
import CardioSessionContent, { CardioSessionContentProps } from "./CardioSessionContent";

interface CardioSessionModalProps extends CardioSessionContentProps {
  onClose: () => void;
}

export default function CardioSessionModal({ onClose, ...contentProps }: CardioSessionModalProps) {
  return (
    <div
      ...
      <div className="min-h-screen flex items-start sm:items-center justify-center p-4 py-10">
        ...
        >
          ...
          <div className="bg-[#f4f4f8] rounded-3xl shadow-2xl p-4 max-h-[85vh] overflow-y-auto">
            <CardioSessionContent {...contentProps} compact />
          </div>
        </div>
      </div>
    </div>
