...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Call the axios-based login function
      const data = await login(formData.email, formData.password);

      // Token is already saved inside login() function
      // Optional: store user info (recommended)
      if (data.user) {
        setAuthUser({
          id: data.user.id,
          name: data.user.name,
      ...
      // router.refresh(); // optional – only if needed for server components
    } catch (err: any) {
      // err is Error object thrown from login()
