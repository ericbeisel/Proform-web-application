...
export default function GoalsPreferencesPage() {
  ...
  const [trainingGoalsOptions, setTrainingGoalsOptions] = useState<
    TrainingGoalOption[]
  >([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (
          savedData.primaryGoal ||
          savedData.trainingGoals ||
          savedData.preferredActivities
        ) {
          setFormData({
  ...
  const toggle = (
    key: "trainingGoals" | "preferredActivities",
    val: string,
  ) => {
    setFormData((prev) => ({
      ...prev,
    ...
    <>
      <SplitLayout
        leftContent={{
          title: "Goal & Preferences",
          description:
            "Tell us what you want to achieve so we can create the perfect training plan for you.",
        }}
        showProgress
        progressData={{
          currentStep: 2,
      ...
      <div className="mb-8 sm:mb-10">
        ...
        <p className="text-gray-500 text-sm sm:text-base">
          Select your primary goal and any additional preferences
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-7">
        ...
        <div>
          ...
          {formData.trainingGoals.length > 0 && (
            <p className="text-xs text-[#6202AC] mt-2">
              {formData.trainingGoals.length} goal
              {formData.trainingGoals.length > 1 ? "s" : ""} selected
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {formData.preferredActivities.length > 0 && (
            <p className="text-xs text-[#6202AC] mt-2">
              {formData.preferredActivities.length} activit
              {formData.preferredActivities.length > 1 ? "ies" : "y"} selected
            </p>
          )}
        </div>
