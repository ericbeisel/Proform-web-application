...
export default function CoreMetricsPage() {
  ...
  useEffect(() => {
    ...
    if (
      savedData.currentWeight ||
      savedData.goalWeight ||
      savedData.heightFeet ||
      savedData.heightInches ||
      savedData.bodyFatPercentage
    ) {
      setFormData({
  ...
  const isFormValid = () => {
    if (!formData.currentWeight || !formData.goalWeight || !formData.heightFeet)
      return false;

    // Additional validation for inches (if entered, must be <= 11)
    if (formData.heightInches && Number(formData.heightInches) > 11)
      return false;
    ...
    <>
      <SplitLayout
        ...
        progressData={{
          currentStep: 3,
          totalSteps: 9,
          nextStep: "Lifestyle Metrics",
        }}
      />

      <div className="mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-black mb-2">
          Let&apos;s track your starting point
        </h1>
        <p className="text-gray-500 text-sm">
          Using{" "}
          <span className="font-semibold text-[#6202AC]">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 mb-1">
            <Upload size={13} className="text-[#6202AC]" />
            Upload BCA Report (Optional)
          </label>
          <p className="text-xs text-gray-400 mb-2">
            Upload your Body Composition Analysis report from InBody or similar
            machines
          </p>
          <label
            htmlFor="bca-upload"
          ...
          >
            ...
            {bcaFile ? (
              <span className="text-sm text-[#6202AC] font-medium">
                {bcaFile.name}
              </span>
            ) : (
              <>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 mb-2">
              <Weight size={13} className="text-[#6202AC]" />
          ...
          <div>
            <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 mb-2">
              <Target size={13} className="text-[#6202AC]" />
        ...
        <div>
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 mb-2">
            <Ruler size={13} className="text-[#6202AC]" />
            Height (feet & inches)*
          </label>
        ...
        <div>
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 mb-2">
            <Percent size={13} className="text-[#6202AC]" />
          ...
          <p className="text-xs text-gray-400 mt-1">
            If you know your current body fat percentage
          </p>
        </div>
        ...
        <div className="bg-purple-50 border border-purple-100 rounded-2xl p-3">
          <p className="text-xs text-purple-900">
            <span className="font-semibold">· Tip:</span> These metrics help us
            create a personalized workout and nutrition plan tailored to your
            goals.
          </p>
        </div>
