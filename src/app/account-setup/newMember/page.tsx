...
export default function NewMemberChecklist() {
  ...
  useEffect(() => {
    const checkAccountSetup = async () => {
      try {
        ...
        const isSetupComplete = details.accountsetup === "1" || 
  ...
  const steps = [
    ...
    { id: 2, title: 'Account Setup', description: 'Build your experience', icon: Settings, status: accountSetupComplete ? 'completed' : 'current' },
  ];

  // Only add step 3 if account setup is complete
  if (accountSetupComplete) {
    steps.push({ id: 3, title: 'Create Team', description: 'Earn points and challenge friends', icon: Users, status: 'current' });
  }
      ...
      {showSuccessMessage && (
        <div className="mb-4 bg-green-50 border border-green-200 rounded-xl p-4 animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-green-800">Account Setup Complete!</p>
              <p className="text-xs text-green-600">Great job! Now let's get you connected with a team.</p>
            </div>
          </div>
        </div>
      )}
          ...
          return (
            <div key={step.id}>
              <div className={`relative flex items-center gap-4 sm:gap-5 px-4 sm:px-6 py-4 sm:py-5 rounded-2xl transition-all
                ${isCurrent ? 'border-2 border-cyan-400 bg-white shadow-sm' : 
                  isCompleted ? 'bg-purple-50 border border-purple-100' : 
