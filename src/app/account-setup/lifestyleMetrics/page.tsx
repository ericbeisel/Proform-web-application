...
export default function LifestyleMetricsPage() {
  ...
  const toast = useToast(); // ← Changed here – most common & correct pattern
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({ ...prev, progressPhoto: file }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.cardioCalorieGoal) return;

    setIsSubmitting(true);

    // Small delay to show loader (remove in production if unwanted)
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-800 mb-2 sm:mb-3">
              <Footprints size={15} className="text-[#6202AC]" />
          ...
          <div>
            <label className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-800 mb-2 sm:mb-3">
              <Flame size={15} className="text-[#6202AC]" />
              Cardio Calorie Goal / Week*
            </label>
            <NumberInput
