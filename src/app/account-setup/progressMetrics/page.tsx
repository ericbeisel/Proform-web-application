  ...
  return (
    <>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm sm:text-base">Upload a photo to visually track your transformation alongside your metrics.</p>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); router.push('/account-setup/yourSchedule'); }} className="space-y-5">
        <div>
          ...
          <div className="border-2 border-dashed border-gray-200 rounded-2xl bg-[#FAFAFA] overflow-hidden">
            ...
            {photoPreview ? (
              <div className="flex flex-col items-center gap-4 p-6 sm:p-8">
                <img src={photoPreview} alt="Progress preview" className="w-32 h-32 sm:w-40 sm:h-40 object-cover rounded-2xl shadow-md" />
                <button type="button" onClick={() => { setPhotoPreview(null); setProgressPhoto(null); }} className="text-sm text-red-500 hover:text-red-600 font-medium">Remove Photo</button>
              </div>
            ) : (
              <label htmlFor="progress-photo" className="flex flex-col items-center justify-center gap-3 py-14 sm:py-20 cursor-pointer px-4">
        ...
        <button type="button" onClick={() => router.push('/account-setup/yourSchedule')} className="w-full text-[#6202AC] font-semibold text-sm sm:text-base py-2 hover:text-[#4e0288] transition-colors">
          Skip For Now
        </button>
      </form>
    </>
