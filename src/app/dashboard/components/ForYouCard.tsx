...
export default function ForYouCard({ 
  ...
  const carouselItems = [
    ...
    {
      icon: Target,
      title: currentWeight ? `Current: ${currentWeight} ${measurementUnit}` : 'Track Weight',
      description: goalWeight ? `Goal: ${goalWeight} ${measurementUnit}` : 'Set your weight goals',
  ...
  return (
    <div className="bg-white rounded-2xl shadow border border-[#e8e6f0] overflow-hidden">
      ...
      <div className="m-3 mb-4 bg-[#1c1929] rounded-xl p-7 text-center relative">
        ...
        <div className={`w-14 h-14 mx-auto mb-3 bg-gradient-to-br ${currentItem.color} rounded-full flex items-center justify-center`}>
          <currentItem.icon size={24} className="text-white" />
        </div>
        <div className="text-white font-bold text-base">{currentItem.title}</div>
        <div className="text-white/50 text-xs mt-1.5">{currentItem.description}</div>
