...
export default function FeedSettingsModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="relative w-full max-w-lg bg-[#f4f4f8] rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Scrollable content */}
        <div className="overflow-y-auto px-4 py-5 space-y-5 flex-1">
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-5 pt-3 pb-5 flex gap-3">
              <button onClick={enableAll}  className="flex-1 text-[13px] font-bold text-purple-600 bg-purple-50 border border-purple-100 py-2.5 rounded-2xl hover:bg-purple-100 transition">Enable All</button>
              <button onClick={disableAll} className="flex-1 text-[13px] font-bold text-gray-500 bg-gray-50 border border-gray-200 py-2.5 rounded-2xl hover:bg-gray-100 transition">Disable All</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 pt-5 pb-3 flex items-center gap-3">
              <span className="w-7 h-7 rounded-full bg-[#6c3fef] flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0">2</span>
              <div>
                    ...
                    {NOTIF_OPTIONS.map(({ val, Icon: NIcon }) => (
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition ${
                          notifs[key] === val ? "bg-[#6c3fef] text-white shadow-sm" : "text-gray-500 hover:text-gray-700"
                        }`}
                      >
                        <NIcon size={11} />
                        {val}
